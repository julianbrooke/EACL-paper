...
\begin{algorithm}
	\caption{Optimization algorithm. $T$ is an ordered list of the nodes in the lattice. Nodes (designated by $t$) contain pointers to the nodes immediately linked to them in the lattice. States (designated by $s$) indicate whether each node is $\mathrm{ON}$ or $\mathrm{OFF}$. Explainedness values are indicated by $e$.}
	\label{alg:opt}
	\begin{algorithmic}
	
	\Function{LocalOpt}{$s_{\mathit{start}}, t, T_{\mathit{rev}}, T_{\mathit{aff}}$}
		\State $s_{\mathit{start}} \gets \Call{Set}{s, t, \mathrm{ON}}$
		...
		\Repeat
			...
			\State $e_{\mathit{curr}} \gets \Call{CalcExpl}{s_{\mathit{curr}}, T_{\mathit{aff}}}$
			\For{$t_{\mathit{rev}} \textrm{ in } T_{\mathit{rev}}$}
					\State $s_{\mathit{new}} \gets \Call{Set}{s_{\mathit{curr}},t_{\mathit{rev}}, \mathrm{OFF}}$
					\State $e_{\mathit{new}} \gets\Call{CalcExpl}{s_{\mathit{new}}, T_{\mathit{aff}}}$
	...
	\EndFunction
\State
\State $\Call{FrequencySortReverse}{T}$
\State $s \gets \Call{InitializeAllOff}{T}$
\Repeat
	\State $\mathit{Changed} \gets \mathrm{FALSE}$
		\For{$t \textrm{ in } T$}
		\State $T_{\mathit{rev}} \gets \Call{GetRelevant}{t,T}$
		\State $T_{\mathit{aff}} \gets \Call{GetAffected}{T_{\mathit{rev}},T}$
		\State $s_{\mathit{new}} \gets \Call{LocalOpt}{s,t,T_{\mathit{rev}},T_{\mathit{aff}}}$
		\If{$s_{\mathit{new}} \neq s$}
