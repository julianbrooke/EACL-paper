...
\begin{algorithm}
	\caption{Optimization algorithm. $N$ is an ordered list of the nodes in the lattice. Nodes (designated by $n$) contain pointers to the nodes immediately linked to them the lattice. States (designated by $s$) indicate whether each node is $ON$ or $OFF$. Explainedness values are indicated by $e$.}
	\label{alg:opt}
	\begin{algorithmic}
	
	\Function{LocalOpt}{$s_{start}, n, N_{rev}, N_{af\hspace{-0.1em}f}$}
		\State $s_{start} \gets \Call{SetNode}{s, n, ON}$
		...
		\Repeat
			...
			\For{$n_{rev} \textrm{ in } N_{rev}$}
					\State $s_{new} \gets \Call{SetNode}{s_{curr}, OFF}$
...
Though the model presented here was designed specifically for FS extraction, we note that it could be useful for similar tasks such as unsupervised creation of morphological lexicons for agglutinative languages. In particular, character or phoneme $n$-grams could compete in an identically structured lattice (though without gapped $n$-grams) to be chosen as the best morphemes for the language, and we could straightforwardly adapt LPR to use, as the denominator of the ratio, phonological predictably (\ie based on vowel/consonant ``tags'') instead of syntactic predictability. It is likely, though, that further modifications would be necessary to target morphological phenomena, and we leave this for future work.
