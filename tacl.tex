...
%\footnote{Recall that our method involves lemmatization, in keeping with which all examples will be presented pre-lemmatized.}
...
With respect to speed, although the optimization of the lattice is several orders of magnitude more time consuming than the decomposition heuristics of \newcite{Brooke15b}, the time needed to build and optimize the lattice was still only a fraction of the time required to collect the statistics necessary to calculate LPR, a necessary first step for both methods: therefore, the overall effect of adding the lattice optimization is fairly minimal. We note that for all 4 corpora, the lattice optimization algorithm converged within 10 iterations.
