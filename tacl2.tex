...
Typically multiword lexicons are created by ranking $n$-grams according to an association measure and applying a threshold. The algorithm of \newcite{Silva99} is somewhat more sophisticated, in that it identifies the local maxima of association measures across subsuming $n$-grams within a sentence to identify MWEs of unrestricted length and syntactic composition; its effectiveness beyond noun phrases, however, seems relatively limited \cite{Ramisch12}. Brooke et \al \shortcite{Brooke14a,Brooke15b} developed a heuristic method intended for general FS extraction in larger corpora, first using conditional probably statistics to do an initial (single pass) coarse-grained segmentation of the corpus, followed by a pass through the resulting vocabulary, breaking larger units into smaller ones based on a tradeoff between marginal and conditional statistics. Work of \newcite{Newman12} is an example of an unsupervised approach which does not use association measures: it extends the Bayesian word segmentation approach of \newcite{Goldwater09} to multiword, applying a generative Dirichlet Process model which jointly constructs a segmentation of the corpus and a corresponding multiword vocabulary.
...
\begin{algorithm}
	\caption{Optimization algorithm. $N$ is an ordered list of the nodes in the lattice. Nodes (designated by $n$) contain pointers to the nodes immediately linked to them in the lattice. States (designated by $s$) indicate whether each node is $\mathrm{ON}$ or $\mathrm{OFF}$. Explainedness values are indicated by $e$.}
	\label{alg:opt}
	\begin{algorithmic}
	
	\Function{LocalOpt}{$s_{\mathit{start}}, n, N_{\mathit{rev}}, N_{\mathit{aff}}$}
		\State $s_{\mathit{start}} \gets \Call{Set}{s, n, \mathrm{ON}}$
		\State $Q \gets \Call{EmptyQueue}{\null}$
		\State $e_{\mathit{best}} \gets 0$
		\State $s_{\mathit{best}} \gets \mathrm{NULL}$
		\State $\Call{Push}{Q, s_{\mathit{start}}}$
		\Repeat
			\State $s_{\mathit{curr}} \gets \Call{Pop}{Q}$
			\State $e_{\mathit{curr}} \gets \Call{CalcExpl}{s_{\mathit{curr}}, N_{\mathit{aff}}}$
			\For{$n_{\mathit{rev}} \textrm{ in } N_{\mathit{rev}}$}
					\State $s_{\mathit{new}} \gets \Call{Set}{s_{\mathit{curr}},n_{\mathit{rev}}, \mathrm{OFF}}$
					\State $e_{\mathit{new}} \gets\Call{CalcExpl}{s_{\mathit{new}}, N_{\mathit{aff}}}$
					\If{$ e_{\mathit{new}} > e_{\mathit{curr}}$}
				
					\State $\Call{Push}{Q, s_{\mathit{new}}}$
					\If{$e_{\mathit{new}} > e_{\mathit{best}}}$:
						\State $e_{\mathit{best}} \gets e_{\mathit{new}}$
							\State $s_{\mathit{best}} \gets s_{\mathit{new}}$
					\EndIf
					\EndIf
				\EndFor
		\Until{$\Call{IsEmpty}{Q}$}
	\State \Return $s_{\mathit{best}}$
	\EndFunction
\State
...
\Repeat
	\State $\mathit{Changed} \gets \mathrm{FALSE}$
		\For{$n \textrm{ in } N$}
		\State $N_{\mathit{rev}} \gets \Call{GetRelevant}{n,N}$
		\State $N_{\mathit{aff}} \gets \Call{GetAffected}{N_{\mathit{rev}},N}$
		\State $s_{\mathit{new}} \gets \Call{LocalOpt}{s,n,N_{\mathit{rev}},N_{\mathit{aff}}}$
		\If{$s_{\mathit{new}} \neq s$}
			\State $s \gets s_{\mathit{new}}$
...
& & & & \multicolumn{4}{c}{\bf{By Length (\%) }} \\
       \cmidrule(lr){5-8}	
\bf{Lexicon} & \bf{Size} & \bf{POS types} & \bf{Gapped} (\%) & 2 & 3 & 4 & 5+ \\ 
\midrule
